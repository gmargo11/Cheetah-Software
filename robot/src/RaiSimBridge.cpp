/*! @file RaiSimBridge.cpp
 *  @brief  The RaiSimBridge runs a RobotController and connects it to RaiSim.
 */

#include "RaiSimBridge.h"
...
void RaiSimBridge::run_controller() {
  
  // init Quadruped Controller

    printf("[Simulation Driver] Running Control Step...\n");
    bool firstRun = true;
    // next robot controller run
    _iterations++;
    runRobotControl();
 ...
 */
void RaiSimBridge::handleControlParameters() {
  ...
  if (request.requestNumber <= response.requestNumber) {
    // nothing to do!
    printf(
        "[RaiSimBridge] Warning: the simulator has run a ControlParameter "
 ...
 */
void RaiSimBridge::runRobotControl() {
  if (_firstControllerRun) {
    ...
    sbus_thread = new std::thread(&RaiSimBridge::run_sbus, this);
  }
  _robotRunner->run();
}
