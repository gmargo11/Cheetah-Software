...
template<typename T>
void FSM_State_NeuralLocomotion<T>::handleLocalization(const lcm::ReceiveBuffer* rbuf, 
...
template<typename T>
void FSM_State_NeuralLocomotion<T>::handleHeightmapLCM(const lcm::ReceiveBuffer *rbuf,
...
template<typename T>
void FSM_State_NeuralLocomotion<T>::handleIndexmapLCM(const lcm::ReceiveBuffer *rbuf,
    const std::string &chan,
 ...
 */
template <typename T>
void FSM_State_NeuralLocomotion<T>::run() {
  if(_b_localization_data){
    _updateStateEstimator();
  }
  // Call the locomotion control logic for this iteration
  Vec3<T> des_vel; // x,y,z
  Vec3<T> des_rpy_vel; // r,p,y
  Vec2<T> des_fp_rel[4];
  Vec4<T> des_fh_rel;
  Vec4<T> des_contact;
  ...
  _LocomotionControlStep(des_vel, des_rpy_vel, des_fp_rel, des_fh_rel, des_offsets, des_durations);
...
template <typename T>
void FSM_State_NeuralLocomotion<T>::_RCLocomotionControl() {
...
template<typename T>
void FSM_State_NeuralLocomotion<T>::_updateStateEstimator(){
...
template<typename T>
void FSM_State_NeuralLocomotion<T>::_JPosStand(){
  Vec3<T> stand_jpos;
...
template <typename T>
void FSM_State_NeuralLocomotion<T>::_UpdateObstacle(){
  _obs_list.clear();
  // TEST
  //Vec3<T> test1; 
  //test1<< 0.7, 0.20, 0.4;
...
template <typename T>
void FSM_State_NeuralLocomotion<T>::_print_obstacle_list(){
...
template <typename T>
void FSM_State_NeuralLocomotion<T>::_Visualization(const Vec3<T> & des_vel){
 ...
 */
template <typename T>
FSM_StateName FSM_State_NeuralLocomotion<T>::checkTransition() {
  // Get the next state
  iter++;
 ...
 */
template <typename T>
TransitionData<T> FSM_State_NeuralLocomotion<T>::transition() {
  // Switch FSM control mode
 ...
 */
template <typename T>
void FSM_State_NeuralLocomotion<T>::onExit() {
 ...
 */
template <typename T>
void FSM_State_NeuralLocomotion<T>::_LocomotionControlStep(const Vec3<T> & des_vel, const Vec3<T> & des_rpy_vel, const Vec2<T> (& des_fp_rel)[4], const Vec4<T> des_fh_rel, const Vec4<int> & des_offsets, const Vec4<int> & des_durations) {
  // StateEstimate<T> stateEstimate = this->_data->_stateEstimator->getResult();

  // Contact state logic
  neural_MPC.run<T>(*this->_data, des_vel, des_rpy_vel, des_fp_rel, des_fh_rel, des_offsets, des_durations, 0.06, 15, _height_map, true, false, true);
